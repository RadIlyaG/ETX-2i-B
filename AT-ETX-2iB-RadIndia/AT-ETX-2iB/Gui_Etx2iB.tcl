...
#   pack $lab1 $lab2 -padx 2 -side left
    ...
    pack $frCommon -fill both -expand 0 -padx 2 -pady 0 -side left 
...
#     catch {exec taskkill.exe /im mb.exe /f /t}
    ...
    set gaSet(plEn) 0
    if {$ret==0} {
      if {[string match {*SetToDefaultAllxxxxx*} $gaSet(startFrom)]==0} {
        ## there is no such test - SetToDefaultAllxxxxx , so perform it always
        set gRelayState red
        IPRelay-LoopRed
