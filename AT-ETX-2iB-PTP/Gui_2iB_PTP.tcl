...
proc GUI {} {
  ...
  set frChooseScope [frame [$mainframe getframe].frChooseScope -bd 2 -relief groove]  
    set fr [frame $frChooseScope.fr1 -bd 0 -relief groove] 
      set gaGui(butScan) [Button $fr.butScan -text "Scan \nScopes" -command {ReadExistScops start}] 
      pack $gaGui(butScan) -padx 2 -pady 2
    pack $fr -anchor w  -padx 2 -pady 2 -side left 
    set fr [frame $frChooseScope.fr2 -bd 0 -relief groove] 
      foreach scp {0 1} scpName {A B} {
        set gaGui(lab0$scp) [Label $fr.lab0$scp -relief groove -bd 0 -text "Scope $scpName "] 
        set gaGui(labSc$scp) [Label $fr.labSc$scp -relief groove -bd 0] 
        set gaGui(cmbSc$scp) [ComboBox $fr.cmbSc$scp -values {1PPS SyncE} -width 7 -textvariable gaSet(scp$scp)]  
        grid $gaGui(lab0$scp) $gaGui(labSc$scp) $gaGui(cmbSc$scp) -padx 2 -pady 2
      }
  ...
  set frTestPerf [frame [$mainframe getframe].frTestPerf -bd 2 -relief groove]     
    ...
    pack $frChooseScope $frTestPerf -fill both -expand yes -padx 2 -pady 2 -anchor nw	 
  ...
  bind . <Control-s> {ReadExistScops start}

  .menubar.tterminal entryconfigure 0 -label "UUT: COM $gaSet(comDut)"
  .menubar.tterminal entryconfigure 1 -label "ETX-205 AUX1: COM $gaSet(comAux1)"     
  .menubar.tterminal entryconfigure 2 -label "ETX-205 AUX2: COM $gaSet(comAux2)"  
...
#     regexp {div ([\d\.]+) \/div} $a m date
  ...
  DialogBox -title "About the Tool" -icon info -type ok  -font {{Lucida Console} 9} -message "ATE software upgrade\n$date"
...
#   }
  ...
  set ret 0 ; #[GuiReadOperator]
  parray gaSet *arco*
  parray gaSet *rato*
  
  foreach scp {0 1} scpLett {A B} {
    set scpLabTxt [$gaGui(labSc$scp) cget -text]
    puts "scpLett:<$scpLett> scpLabTxt:<$scpLabTxt>"    
    if {$scpLabTxt==""} {
      set gaSet(fail) "Scope $scpLett does not connected"
      set ret -1
    }  
  }
  foreach scp {0 1} scpLett {A B} {
    set scpCmbTxt$scp [$gaGui(cmbSc$scp) cget -text]
    puts "scpLett:<$scpLett> scpCmbTxt:<[set scpCmbTxt$scp]>"    
    if {[set scpCmbTxt$scp]==""} {
      set gaSet(fail) "Scope $scpLett does not connected to any Test"
      set ret -1
    }  
  }
...
proc Quit {} {
  ...
  if {$ret=="yes"} {SQliteClose; CloseRL; exit}
}
