...
proc GUI {} {
  ...
  set descmenu {
    ...
    "&Tools" tools tools 0 {	  
      ...
      {separator}  
      {cascad "Power" {} pwr 0 {
        {command "PS ON" {} "" {} -command {GuiPower $gaSet(pair).1 1}} 
        {command "PS OFF" {} "" {} -command {GuiPower $gaSet(pair).1 0}} 
        {command "PS OFF and ON" {} "" {} \
            -command {
    ...
    "&Terminal" terminal tterminal 0  {
       ...
       {command "ETX" "" "" {} -command {OpenTeraTerm gaSet(comGen1)}}       
     }
    "&About" all about 0 {
...
# {command "Update INIT and UserDefault files on all the Testers" {} "Exit" {} -command {UpdateInitsToTesters}}
  ...
  pack $lab1 $lab2 -padx 2 -side left
    ...
    set frCommon [frame $mainframe.frCommon  -bd 2 -relief groove]
      ...
      set gaGui(enUsbTest) [checkbutton $frCommon.chb2 -text "Perform Usb Test" \
          -variable gaSet(enUsbTest) -command {BuildTests}]
      set gaGui(enReadTrace) [checkbutton $frCommon.chb3 -text "Read Traceability" \
          -variable gaSet(readTrace)]
      set gaGui(enPart1Test) [checkbutton $frCommon.chb4 -text "Perform Partial Tests 1" \
          -variable gaSet(enPart1Test) -command {BuildTests}]    
      grid $gaGui(enDownloadsBefore) -sticky w
      grid $gaGui(enUsbTest) -sticky w
      grid $gaGui(enReadTrace) -sticky w
      grid $gaGui(enPart1Test) -sticky w
      ...
      #grid $frTsts 08/12/2016 08:00:21
    ...
    set frDUTa [frame [$mainframe getframe].frDUT -bd 0 -relief groove] 
      set frDUT  [frame $frDUTa.frDUT -bd 2 -relief groove] 
        ...
        set gaGui(entDUT) [Entry $frDUT.entDUT -bd 1 -justify center -width 50\
              -editable 1 -relief groove -textvariable gaSet(entDUT) -command {ScanUutBarcode}\
      ...
      set frSerNum  [frame $frDUTa.frSerNum -bd 0 -relief groove]
        set gaGui(enSerNum) [checkbutton $frSerNum.chbEnSerNum -text "Insert Ser. Number" \
            -variable gaSet(enSerNum) -command ToggleEnSerNum]
        pack $gaGui(enSerNum)  -side left -padx 2   
    
      pack $frDUT -side left -padx 2 -pady 2   
      pack $frSerNum -side right -padx 2 -pady 2   
...
#       set f [$frTestPerf getframe]      17/09/2014 16:26:46
  ...
  .menubar.tterminal entryconfigure 1 -label "ETX: COM $gaSet(comGen1)"
...
#     regexp {div ([\d\.]+) \/div} $a m date
    ...
    #set d1 [string trim $date]
#     foreach {m na da} [regexp -all  -inline {\s?([\w\s\/\-]+):\s+([\d\.]+)\s?} $date] {
#       puts "<[string trim $na]> <$da>"
#     }
    
#     set txt "\n"
#     foreach {m na da} [regexp -all  -inline {\s?([\w\s\/\-]+):\s+([\d\.]+)\s?} $date] {
#       append txt "[format "%-20s" [string trim $na]] $da\n"
#     }
#     set date $txt
...
#           -message "ETX-2i-B/V: the software upgrated at 23.03.2017\n\
#           ETX-DNFV-M/R: the software upgrated at 23.03.2017\n\
#           ETX-2i-B/10 Ports: the software upgrated at 01.03.2017\n\
#           ETX-2i-B/6 Ports: the software upgrated at 01.03.2017\n\
#           ETX-2i-B/RJIO: the software upgrated at 01.03.2017\n\
#           ETX-2i-B/Combo: the software upgrated at 01.03.2017"
}
#***************************************************************************
#** ButRun
#***************************************************************************
proc ButRun {} {
  ...
  if {$gaSet(pair)=="5"} {
    for {set pa 1} {$pa <= 27} {incr pa} {
      set gaSet($pa.barcode1.IdMacLink) ""
      set gaSet(serialNum.$pa) ""
    }
  } else {
    set pa $gaSet(pair) 
    set gaSet($pa.barcode1.IdMacLink) ""
...
#   set RunGuiDuration [RLAutoUpdate::GetRunGuiDuration]
#   if {$RunGuiDuration>=24} {
#     DialogBox -message "The Tester is open more then 24 hours\nPress OK to shutdown"\
#       -title "Tester working time" -type OK
#     SaveInit
#     after 1000
#     exit  
#   }
...
#     BuildTests
  ...
  if {$ret==0} {
    ...
    if {$ret=="-4"} {
	    ## do not include GetSerialNumber fail in statistics
      set gaSet(runStatus) ""  
	  }
	  pack $gaGui(frFailStatus)  -anchor w
...
proc GuiInventory {} {  
  ...
  ## 06/12/2016 08:01:45
  if {![file exists uutInits/$gaSet(DutInitName)]} {
    set parL [list sw dbrSW swPack dnfvCPU cpld dbrBVerSw dbrBVer]
    foreach par $parL {
      set gaSet($par) ??
      set gaTmpSet($par) ??
    }
    foreach indx {Boot SW RJIO DGasp ExtClk RTR Default V 2iB DNFV} { 
      set gaSet([set indx]CF)  c:/aa
      set gaTmpSet([set indx]CF)  c:/aa
    }
  }
  
  ## if any parameter is absent, fill it by ?? and aa
  set parL [list sw dbrSW swPack dnfvCPU cpld dbrBVerSw dbrBVer]
  foreach par $parL {
  ...
  if {$b!="DNFV"} {
    ...
    set fr [frame $base.frSwPack -bd 0 -relief groove]
      ...
      pack [Entry $fr.cbSW -justify center -editable 1 -command [list IdBarcode2SW %a %w %k]\
          -textvariable gaTmpSet(swPack)] -pady 1 -padx 2 -anchor w -side left
    pack $fr  -anchor w
    ...
    pack $fr  -anchor w
#     if {$b=="V"} {
#       set fr [frame $base.frDNFVSwVer -bd 0 -relief groove]
#         pack [Label $fr.labSW  -text "DNFV SW Ver" -width 15] -pady 1 -padx 2 -anchor w -side left
#         pack [Entry $fr.cbSW -justify center -width 45 -state disabled -editable 0 -textvariable gaTmpSet(dbrSW)] -pady 1 -padx 2 -anchor w -side left
#       pack $fr  -anchor w
#       set fr [frame $base.frDNFVSwPack -bd 0 -relief groove]
#         pack [Label $fr.labSW  -text "DNFV SW Pack" -width 15] -pady 1 -padx 2 -anchor w -side left
#         pack [Entry $fr.cbSW -justify center -editable 1 -command [list IdBarcode2SW %a %w %k]\
#             -textvariable gaTmpSet(swPack)] -pady 1 -padx 2 -anchor w -side left
#       pack $fr  -anchor w
#     }
    ...
    set fr [frame $base.frSwPack -bd 0 -relief groove]
      pack [Label $fr.labSW  -text "SW Pack" -width 15] -pady 1 -padx 2 -anchor w -side left
      pack [Entry $fr.cbSW -justify center -editable 1 -command [IdBarcode2SW %A %W %K]\
          -textvariable gaTmpSet(swPack)] -pady 1 -padx 2 -anchor w -side left
    pack $fr  -anchor w
    foreach indx {CPU} {
  ...
  if {$gaSet(dutBox)!="DNFV"} {   
    foreach indx {Boot SW RJIO V 2iB DGasp ExtClk RTR Default} {
      if {$indx==$gaSet(dutBox) || \
          $indx=="DGasp" || $indx=="ExtClk" || $indx=="Boot" || $indx=="SW" || \
          $indx=="RTR" || $indx=="Default"} {
        set fr [frame $base.fr$indx -bd 0 -relief groove]
...
proc ButImportInventory {} {
  ...
  if {$fil!=""} {  
    ...
    source $fil
    set parL [list sw dbrSW swPack dnfvCPU cpld]
    foreach par $parL {
      set gaTmpSet($par) $gaSet($par)
    }
...
proc GuiReadSerNum {} {
  ...
  set pairs [PairsToTest]
  set ret [GetSerNum -i images/serNum.32.ico -pairs $pairs]
  if {$ret=="-1"} {
    set gaSet(fail) "No Serial Number"
    return -2
  } else {
    foreach pa $pairs {
      set ser [lindex $ret [expr {1+[lsearch $ret $pa]}]]
      set gaSet(serialNum.$pa) $ser
      AddToPairLog $pa "Serial Number $pa: $ser"
    }
