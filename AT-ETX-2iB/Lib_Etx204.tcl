proc OpenEtxGen {} {
  ...
  if {$ret==0} { 
    ## will be performed in DataTransmissionConf
    #InitEtxGen 1  
  }
  return $ret 
}
...
proc InitEtxGen {gen}  {
  ...
  RLEtxGen::GenConfig $id -updGen all -factory yes -genMode GE -minLen 64 -maxLen 64 \
      -chain 1 -packRate 1000 -packType MAC 
...
# Etx204Check
# ***************************************************************************
proc Etx204Check {lGens packRate box} {
  global gaSet aRes
  ...
  foreach gen {1} {
    ...
    foreach port $lGens {
      ...
      foreach stat {ERR_CNT FRAME_ERR PRBS_ERR SEQ_ERR FRAME_NOT_RECOGN} {
        ## 
        ## 04/04/2019 07:32:42 no check FRAME_NOT_RECOGN in ETX-2I-B/V and DNFV
        if {($box=="V" || $box=="DNFV") && $stat=="FRAME_NOT_RECOGN"} {
          continue
        }
        set res $aRes(id$id,[set stat],Gen$port)
        if {$res!=0} {
          set gaSet(fail) "The $stat in Generator Port-$port is $res. Should be 0"
          set res$port -1
          break
        }
      }
      ...
      foreach stat {PRBS_OK RCV_BPS RCV_PPS} {
        set res $aRes(id$id,[set stat],Gen$port)
        if {$res==0} {
          set gaSet(fail) "The $stat in Generator Port-$port is 0. Should be more"
          set res$port -1
          break
        }
        if {$stat=="RCV_PPS"} {
          ...
          set diffDevRefProc  [expr {$diffDevRef * 100}] 
          if {$diffDevRefProc>1.0} {
            puts "res1:$res1"
            puts "diff:$diff"
            puts "diffDevRefProc:$diffDevRefProc"
            puts "res1:$res1"
            update
            set gaSet(fail) "The $stat in Generator Port-$port is $res. Should be $packRate"
            set res$port -1
            break  
          }
        }
